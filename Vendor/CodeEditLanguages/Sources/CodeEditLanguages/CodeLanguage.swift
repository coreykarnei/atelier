...
public struct CodeLanguage {
    ...
    internal func queryURL(for highlights: String = "highlights") -> URL? {
        // Atelier patch: `.copy("Resources")` lands the folder at different
        // depths depending on who built the bundle. Under Xcode the bundle's
        // resourceURL is its root, so the queries sit at
        // `<root>/Resources/tree-sitter-x/`; under `swift build` resourceURL
        // already *is* `<bundle>/Resources`, and the old fixed path doubled
        // it — every query URL pointed at nothing and no language ever
        // highlighted in a SwiftPM-built app. Probe both.
        guard let resourceURL else { return nil }
        let tail = "tree-sitter-\(tsName)/\(highlights).scm"
        let candidates = [
            resourceURL.appendingPathComponent("Resources/\(tail)"),
            resourceURL.appendingPathComponent(tail),
        ]
