...
open class TerminalView: NSView, NSTextInputClient, NSUserInterfaceValidations, TerminalDelegate {
    ...
    /// Wheel routing, the way a terminal decides it rather than its host:
    /// with mouse reporting on, the wheel reaches the program as button 64/65
    /// reports (it owns its own viewport); on the alternate screen with
    /// alternateScroll (`?1007`) set, it becomes ↑/↓ key presses; otherwise it
    /// scrolls the local scrollback.
    open override func scrollWheel(with event: NSEvent) {
        if allowMouseReporting && terminal.mouseMode != .off {
            forwardWheel(event) { up, hit in
        ...
        if terminal.isCurrentBufferAlternate && terminal.alternateScroll {
            forwardWheel(event) { up, _ in
                send(terminal.applicationCursor ? (up ? EscapeSequences.moveUpApp : EscapeSequences.moveDownApp)
    ...
    /// Fractional trackpad deltas accumulate until they amount to a whole
    /// line, so a slow two-finger drag still moves the program one line at a
    ...
    private func forwardWheel(_ event: NSEvent, _ emit: (_ up: Bool, _ hit: (grid: Position, pixels: Position)) -> Void) {
        ...
        if event.hasPreciseScrollingDeltas {
            ...
            wheelRemainder = 0
            lines = Int(event.scrollingDeltaY.rounded(.awayFromZero))
        }
